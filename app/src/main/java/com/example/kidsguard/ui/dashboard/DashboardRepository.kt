...
class DashboardRepository(
    ...
    private val selectedChildId: StateFlow<String?>
) {
    ...
    val dashboardState: Flow<DashboardState> = selectedChildId.flatMapLatest { childId ->
        if (childId == null) {
            flowOf(DashboardState.Error("No child selected"))
        } else {
            combine(
                ...
                syncProvider.getChildStatus(childId),
                syncProvider.getLatestActivity(childId)
            ) { args: Array<Any?> ->
                ...
                DashboardState.Success(
                    DashboardUiModel(
                        ...
                        isMockChild = childId.contains("mock")
                    )
                )
            }
        }
