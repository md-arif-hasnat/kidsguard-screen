...
import { ConfigRepository, UpdateConfig, ReleaseChannel, AppRelease } from '@/lib/repositories/ConfigRepository';
  ...
  return (
    <DashboardLayout>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Release Manager</h1>
          <p className="text-slate-500 mt-1">Deploy and manage application updates across all channels.</p>
        </div>
        <div className="flex gap-2">
            <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1.5 border border-slate-200">
                ...
                <form onSubmit={handlePublish} className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Version Name</label>
                            <input
                                type="text"
                                value={versionName}
                                onChange={e => setVersionName(e.target.value)}
                                placeholder="1.0.0"
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary-500 outline-none font-bold"
                            />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Version Code</label>
                            <input
                                type="number"
                                value={versionCode}
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">APK Download URL</label>
                            <input
                                type="url"
                                value={apkUrl}
                                onChange={e => setApkUrl(e.target.value)}
                                placeholder="https://github.com/.../release.apk"
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary-500 outline-none font-medium"
                            />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Release Channel</label>
                            <select
                                value={channel}
                                onChange={e => setChannel(e.target.value as any)}
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary-500 outline-none font-bold appearance-none"
                            >
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Mandatory Update</label>
                            <div className="flex items-center gap-4 h-[52px] bg-slate-50 border border-slate-200 rounded-xl px-4">
                                <label className="flex items-center gap-2 cursor-pointer group">
                                    ...
                                    />
                                    <span className="text-sm font-bold text-slate-700 group-hover:text-primary-600 transition-colors">Force Update Required</span>
                                </label>
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">File Size</label>
                            <input
                                type="text"
                                value={fileSize}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Min Android</label>
                            <input
                                type="text"
                                value={minAndroid}
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Update Prompt Message</label>
                            <input
                                type="text"
                                value={message}
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Release Notes</label>
                            <textarea
                                rows={4}
                                value={notes}
                                onChange={e => setReleaseNotes(e.target.value)}
                                placeholder="✓ Added Safe Zone alerts&#10;✓ Fixed battery sync"
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary-500 outline-none font-medium resize-none"
                            />
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-50 flex justify-end">
                        <button
                            disabled={publishing}
                            className="bg-primary-600 hover:bg-primary-700 text-white font-black py-4 px-12 rounded-2xl shadow-xl shadow-primary-200 transition-all flex items-center gap-3 disabled:opacity-50"
                        >
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                    {history.map(rel => (
                        <div key={rel.id} className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50 hover:border-primary-500/50 transition-all group">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <div className="flex items-center gap-2">
                                        <p className="font-bold text-sm">v{rel.latestVersionName}</p>
                                        <span className={clsx(
                            ...
                            <p className="text-[11px] text-slate-400 line-clamp-2 italic">{rel.releaseNotes || 'No notes'}</p>
                            <p className="text-[9px] text-slate-600 mt-2 font-mono">{rel.releasedAt ? new Date(rel.releasedAt.seconds * 1000).toLocaleDateString() : 'Unknown date'}</p>
                        </div>
                    ))}
                </div>
            </section>
